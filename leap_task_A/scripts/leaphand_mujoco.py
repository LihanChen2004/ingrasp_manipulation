...
class Arena:
    def __init__(self):
        ...
        scene = mjcf.from_file(
            os.path.join(
                rospack.get_path("my_robot_description"), "urdf/objects/scene.xml"
            )
        )
        self.model.attach(scene)

        xml_file_path = os.path.join(
            rospack.get_path("my_robot_description"),
            "urdf/leaphand_xml/leaphand_mujoco.xml",
        )
        with open(xml_file_path) as file:
            xml_content = file.read()
        xml_content = xml_content.replace(
            "./leaphand.xml",
            os.path.join(os.path.dirname(xml_file_path), "leaphand.xml"),
        )
        self.leap_hand = mjcf.from_xml_string(xml_content)
        self.model.attach(self.leap_hand)

        xml_file_path = os.path.join(
            rospack.get_path("my_robot_description"), "urdf/objects/cylinder_mujoco.xml"
        )
        with open(xml_file_path) as file:
            xml_content = file.read()
        xml_content = xml_content.replace(
            "./cylinder.xml",
            os.path.join(os.path.dirname(xml_file_path), "cylinder.xml"),
        )
...
class Simulation:
    ...
    def initialMujoco(self):
        ...
        for part_joints_name in list(self.P.part_joints_name.values()):
            for joint_name in part_joints_name:
                self.mujoco_joints[joint_name] = self.arena.leap_hand.find(
                    "joint", joint_name
                )
    ...
    def ctrlHandJointPos(self, target_joint_pos):
        ...
        for i, joint_name in enumerate(self.P.part_joints_name["hand"]):
            self.physics.bind(
                self.mujoco_actuators[joint_name]
            ).ctrl = target_joint_pos[i]
    ...
    def getQRCodePose(self):
        qr_code_center = self.physics.bind(
            self.arena.object.find("site", "QR_code_pos")
        )
...
def test1():
    ...
    urdf_path = os.path.join(
        rospack.get_path("my_robot_description"), "urdf/leaphand_taskA.urdf"
    )
